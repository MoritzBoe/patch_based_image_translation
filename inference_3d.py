...
def create_patch_grid(img_shape, crop_size, overlap):
    ...
    for x in np.arange(0, img_shape[1]-overlap, crop_size-overlap):
        if x == 0:  # first row
            x_new_start = 0
            x_start = x
            x_end = x+crop_size
        elif x+crop_size > img_shape[1]:  # last row when more overlap is needed
            x_new_start = x+overlap          
            x_start = img_shape[1]-crop_size
            x_end = img_shape[1]
        else:  # all other rows
            x_new_start = x+overlap
            x_start = x
            x_end = x+crop_size
        for y in np.arange(0, img_shape[2]-overlap, crop_size-overlap):
            if y == 0:
                y_new_start = 0
                y_start = y
                y_end = y+crop_size
            elif y+crop_size > img_shape[2]:  # last row when more overlap is needed
                y_new_start = y+overlap
                y_start = img_shape[2]-crop_size
                y_end = img_shape[2]
            else:
                y_new_start = y+overlap
                y_start = y
                y_end = y+crop_size
            for z in np.arange(0, img_shape[0]-overlap, crop_size-overlap):
                if z == 0:
                    z_new_start = 0
                    z_start = z
                    z_end = z+crop_size
                elif z+crop_size > img_shape[0]:  # last row when more overlap is needed
                    z_new_start = z+overlap
                    z_start = img_shape[0]-crop_size
                    z_end = img_shape[0]
                else:
                    z_new_start = z+overlap
                    z_start = z
